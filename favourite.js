  ...
  if (myWish.length != 0) {
    ...
    for (e = 0; e <= myWish.length - 1; e++) {
      document.getElementById("fav_wish_main").innerHTML +=
        '<div id="fav_wish' +
        e +
        '" class="detail"><div class="fav_content" style="float: left;"></div><div class="wishDetail btnActive" style="float: right;"><span style="position: relative; top: calc(50% - 65px)">确认<br/>实现</span></div></div>';
      $("#fav_wish" + e).css("display", "block");
      $("#fav_wish" + e + " .fav_content").text(myWish[e]["content"]);
      if (Number(myWish[e]["status"]) == 2) {
        $("#fav_wish" + e + " .wishDetail").attr(
          "onclick",
          "confirmFinish(" + e + "," + myWish[e]["id"] + ")"
        );
      } else {
        $("#fav_wish" + e + " .wishDetail").css("filter", "grayscale(100%)");
      }
    }
  }
}
...
function printMyHelpWish() {
  ...
  if (myHelpWish.length != 0) {
    for (f = 0; f <= myHelpWish.length - 1; f++) {
      document.getElementById("fav_help_wish_main").innerHTML +=
        '<div id="fav_help_wish' +
        f +
        '" class="detail"><div class="fav_content" style="float: left;"></div><div class="wishDetail btn btnActive" style="float: right;">详情</div></div>';
      $("#fav_help_wish" + f + " .fav_content").text(myHelpWish[f]["content"]);
      $("#fav_help_wish" + f + " .wishDetail").attr(
        "onclick",
        "printHelpWishDetail(" + f + ");"
      );
    }
  }
}
...
function printHelpWishDetail(num) {
  ...
  $("#btn_finish_help_wish").attr(
    "onclick",
    "fogHelpWish(" + num + "," + myHelpWish[num]["id"] + ",1)"
  );
  $("#btn_giveup_help_wish").attr(
    "onclick",
    "fogHelpWish(" + num + "," + myHelpWish[num]["id"] + ",0)"
  );
  $("#fav_help_wish_chr").attr(
    "src",
    "img/wish/paper/paper" + myHelpWish[num]["paper"] + ".png"
  );
  goTo("p3");
}

function fogHelpWish(num, id, fog) {
  //fog - finish or give up
  if (fog == 1) {
  ...
  x9.onload = () => {
    if (x9.status != 200) {
      ...
      //         break;
      // }
      newAlert(JSON.parse(x9.responseText).message);
    } else {
      if (fog == 1) {
        newAlert("确认实现成功\n\\撒花/\\撒花/");
      } else {
  ...
  x7.onload = () => {
    if (x7.status !== 200) {
      ...
      if (x7.status == 500) {
        ...
        newAlert(JSON.parse(x7.responseText).message);
      }
